...
class TelaLogin(tk.Frame):
    def __init__(self, parent, controller):
        ...
        botao_cadastrar = tk.Button(center_frame, text="Criar Conta (Cadastrar)", command=lambda: self.controller.show_frame("TelaCadastro"),width=20)
...
class AdicionarTransacaoPopup(Toplevel):
    ...
    def salvar(self):
        resultado = fn.adicionar_transacao(self.data_var.get(),self.tipo_var.get(),self.categoria_var.get(),self.descricao_var.get(),self.valor_var.get())
...
class TelaSistema(tk.Frame):
    def __init__(self, parent, controller):
        ...
        btn_listar_todas = tk.Button(center_frame, text="Listar Todas", command=lambda: ListarTodasPopup(self.master))
